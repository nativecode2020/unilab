...
class Test extends Factory {
    ...
    addRow(test) {
        ...
        this.dataTable.row.add({
            0: test.test_name,
            1: `<span class="row" id="test-${test.hash}">${getRefrences(test.option_test, test.hash)}</span>`,
...
function getRefrences(options, hash) {
    if (options == '{"": ""}' || options == '' || options == '{"component": []}') {
        ...
        newRefrence = newRefrence.map((item, cur) => {
            ...
            return `<span class="badge badge-light border border-info p-2 mr-2 mb-2 col-auto" id="test-${hash}_kit-${(kits.find(x => x.id == item.kit)?.name.replace(/[^a-zA-Z0-9]/g, "_") ?? "No Kit").split(" ").join("_")}">
