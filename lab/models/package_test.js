...
let _tests = run_both(`select hash,test_name as name from lab_test where test_type<>'3';`).result[0].query0.filter(test => {
...
class PackageTest extends Factory {
        ...
        this.dataTable = setServerTable(
            ...
            [
                ...
                },
                {
                    data: 'null',
                    render: function (data, type, row) {
                        return `<span onclick="updateTest('${row.hash}')">${row.device_name ? row.device_name : "NO device"}</span>`
                    }
                },
                {
                    data: 'null',
                    render: function (data, type, row) {
                        return `<span onclick="updateTest('${row.hash}')">${row.kit_name ? row.kit_name : "NO Kit"}</span>`
                    }
                },
                {
                    data: 'null',
                    render: function (data, type, row) {
                        return `<span onclick="updateTest('${row.hash}')">${row.unit_name ? row.unit_name : "NO Unit"}</span>`
                    }
                },
                {
                    "data": null,
    ...
    addRow(row) {
        ...
        if (this.dataTable.row(`#${row.hash}`)[0].length == 0) {
            this.dataTable.row.add({
                ...
                1: row.cost,
                2: row.price,
                3: row.device_name == '' || row.device_name == null ? 'No Device' : row.device_name,
                4: row.kit_name == '' || row.kit_name == null ? 'No Kit' : row.kit_name,
                5: row.unit_name == '' || row.unit_name == null ? 'No Unit' : row.unit_name,
                6: `
                <a class="btn-action" onclick="updateTest('${row.hash}')"><i class="fas fa-edit"></i></a>
                <a class="btn-action delete" onclick="fireSwal(deleteTest, '${row.hash}')"><i class="far fa-trash-alt"></i></a>
                `,
    ...
    mainCondition() {
        return `where lab_package.lab_id='${localStorage.getItem('lab_hash')}' and catigory_id ='9' and  lab_package.isdeleted='0'`;
    }

    pageCondition() {
        return `select count(*) as count from lab_package inner join lab_pakage_tests on lab_package.hash=lab_pakage_tests.package_id where lab_package.lab_id='${localStorage.getItem('lab_hash')}' and catigory_id ='9';`;
    }
...
class Package extends PackageTest {
    ...
    mainCondition() {
        return `where lab_package.lab_id='${localStorage.getItem('lab_hash')}' and catigory_id ='8' and ${this.table}.isdeleted='0'`;
    }
