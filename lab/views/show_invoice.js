...
function manageInvoiceHeight(invoice) {
    ...
    $(invoice).find('.page .center2 .tester .test').each(function () {
    ...
    let invoices = addTestToInvoice(allInvoiceTestsHeight, allTestsElements, cloneInvoice, center2Scroll);
    $(invoice).empty();
...
function addTestToInvoice(allInvoiceTestsHeight, allTestsElements, cloneInvoice, center2Scroll) {
    ...
    if (allTestsElements.length > 0) {
        addTestToInvoice(allInvoiceTestsHeight, allTestsElements, cloneInvoice, center2Scroll)
    }
    return invoices;
}
...
function manageTestType(type, test = {}) {
    let { name, color, result, hash, category, checked, normal, unit, flag, font } = test;
    switch (type) {
        case 'flag':
            return `
            <div data-flag="flag" class="test row m-0 category_${category?.split(' ')?.join('_')} border-test" id="test_normal_${hash}" data-cat="${category?.split(' ')?.join('_')}">
