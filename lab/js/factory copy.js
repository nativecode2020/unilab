...
const NOW = new Date();
const TODAY =
  NOW.getFullYear() + "-" + getMonth(NOW.getMonth()) + "-" + NOW.getDate();
...
const saveCheck = function (element, table, hash) {
  ...
  run(
    `update ${table} set ${element.attr("name")}='${element.attr(
      "value"
    )}' where ${hash}='${element.attr("id")}';`
  );
};
...
const buildInput = function (name, options, label) {
  let __label =
    options.type == "hidden" ? "" : `<label for="${name}">${label}</label>`;
  return `<div class="col-md-${options.size ?? 4} mb-4">
                ${__label}
                <input type="${
                  options.type
                }" name="${name}" class="form-control" id="${name}" placeholder="ادخل ${label}" ${
    options.required
  }>
            </div>`;
};
...
const buildTextarea = function (name, options, label) {
  return `<label for="${name}" class="w-100">${label}</label><div id="editor_${name}" class="col-md-${
    options.size ?? 4
  } mb-4"></div>
...
const buildFileInput = function (name, options, label) {
  return `<div class="col-md-${options.size ?? 4} mb-4">
                ...
                ${
                  options.scoundLabel
                    ? `<label>${options.scoundLabel}</label>`
                    : ""
                }
                <input dir="ltr" type="${
...
const buildTable = function (options) {
  ...
  buildButtons =
    buildButtons ??
    function (tablename, item, item_hash, hashrow, editfields, modalid) {
      return `<ul class="table-controls">
            <li>
                <a class="bs-tooltip" onclick="editItem('${item_hash}','${hashrow}','${tablename}','${editfields}','${
        modalid ?? null
  ...
  fetchData =
    fetchData ??
    function (tableFields = fields, table = table) {
      query = "";
  ...
  buildHeader =
    buildHeader ??
    function (tableFields = fields, deleteFields = deleteFields) {
      headerFields = deleteFields
        ? Object.keys(tableFields).reduce((acc, cur) => {
  ...
  buildBody =
    buildBody ??
    function (tableDate = data, tableFields = headerFields, styleFields) {
      $(body).empty();
      bodyHtml = "";
                      ...
                      modal.size
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-primary submit-form" data-form="${encodeURIComponent(
  ...
  editItem =
    editItem ??
    function (hashfield, hash, table, query, modalId) {
      let data = run(
        `select ${query} from ${table} where ${hashfield}='${hash}';`
      ).result[0].query0[0];
  ...
  saveItem =
    saveItem ??
    function (form, modalId, tableName) {
      function save() {
        preSave();
        let formFields = "";
        form = form.data("form");
        ...
        if (itemHash == 0) {
          run(
            `insert into ${tableName}(${Object.keys(form).join(
              ","
          ...
          run(
            `update ${tableName} set ${Object.keys(form)
              .map((x) => `${x}='${$(`#${x}`).val()}'`)
              .join(",")} where hash=${itemHash};`
          );
        }
        $(`#${modalId}`).modal("toggle");
        reset_form(modalId);
  ...
  const returnObj = {
    ...
    render: function () {
      buildBody(
        fetchData(this.fields, this.table),
        this.headerFields,
        this.style
      );
    },
