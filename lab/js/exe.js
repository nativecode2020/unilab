...
function upload(dataForm) {
    ...
    try {
        $.ajax({
            ...
            error: function (err) {
                console.log('%c========== Error  ==========', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px');
                console.log('%c========== Error  ==========', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px');
                console.log('=====>', err);
            }
        });
    ...
    Swal.fire({
        ...
        .then(async result => {
            ...
            if (result.value) {
                ...
                fromData.append('date', run(`select insert_record_date as date from system_users_type order by id desc limit 1;`).result[0]?.query0?.[0]?.date ?? '2023-01-01 00:00:00');
                let queries = await fetch('http://umc.native-code-iq.com/app/index.php/Offline/getAsyncData', {
                    method: 'POST',
                    headers: {},
                    body: fromData
                ...
                run(`insert into system_users_type (title,insert_record_date) values ('update by ${localStorage.getItem('name') ?? ''}','${new Date().toISOString().slice(0, 19).replace('T', ' ')}');`);
            }
        })
        .then(async () => {
            body.removeChild(document.getElementById("alert_screen"));
