...
class Query {
    ...
    getAll(page = 0) {
        return run_both(this.getQuery(`${this.mainCondition()} ${this.groupByQuery()} ${this.orderByQuery()} ${this.limitQuery(page)}`)).result[0].query0;
    }

    getItem(hash) {
        return run_both(this.getQuery(this.itemQuery(hash))).result[0].query0[0];
    }
}
...
class Factory extends Query {
    constructor(table, tableLabel, fields, options = {}) {
        ...
        if (this.pageCondition() != '') {
            this.size = run_both(`${this.pageCondition()};`).result[0].query0[0].count ?? 0;
        } else {
            this.size = 0;
        }
        this.pages = [0];
        ...
        <script>
            ...
            $('#${this.table}-table_wrapper input[type=search]').off().on('keyup', function(e) {
                ...
                if(${this.table}.havingQuery('') != ''){
                    if($(this).val()){
                        let dataBaseLabs = run_both(${this.table}.searchQuery($(this).val())).result[0].query0;
                        ...
                        ${this.table}.resetPageForSearch(${run_both(`${this.pageCondition()};`).result[0].query0[0].count ?? 0});
                        customPagination.call(${this.table});
                    }
                }
            });
    ...
    // getUpdateItem(hash){
    //     return run_both(`select ${this.fields.map(field => field.name).join(',')} from ${this.table} where hash='${hash}';`).result[0].query0[0];
    // }
    ...
    filterTable(user_q, start_date, end_date) {
        // get the data
        let data = run_both(this.getQuery(`${this.filterQuery(user_q, start_date, end_date)}`)).result[0].query0;
        // clear the table
        this.dataTable.clear();
        // add the rows
        this.createTableBody(data);
        ...
        dbReq.onsuccess = e => {
            ...
            req.onsuccess = e => {
                if (e.target.result?.[0]) {
                    ...
                    let data = run_both(options.query).result[0].query0;
                    for (let i of data) {
                        _req.add(i);
                    }
                    resolve(data);
