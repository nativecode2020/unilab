...
const NOW = new Date();
const TODAY = NOW.getFullYear() + '-' + getMonth(NOW.getMonth()) + '-' + getMonth(NOW.getDate(), false);
const firstDayOfMonth = new Date(NOW.getFullYear(), NOW.getMonth(), 1);
const lastDayOfMonth = new Date(NOW.getFullYear(), NOW.getMonth() + 1, 0);
const FIRSTDAYOFMONTH = firstDayOfMonth.getFullYear() + '-' + getMonth(firstDayOfMonth.getMonth()) + '-' + getMonth(firstDayOfMonth.getDate(), false);
const LASTDAYOFMONTH = lastDayOfMonth.getFullYear() + '-' + getMonth(lastDayOfMonth.getMonth()) + '-' + getMonth(lastDayOfMonth.getDate(), false);
...
function customPagination() {
    ...
    for (let i = 0; i < this.numberOfPages ?? 5; i++) {
        pages += ` <li class="page-item ${i == 0 ? "active" : ''} ${i >= 2 ? 'd-none' : ''} page_${i}"  onclick="goToPage.call(${this.table}, '${i}')" style="cursor: pointer;"><a class="page-link" >${i + 1}</a></li>`;
    }
...
function goToPage(page) {
    ...
    $(`#${this.tableId}_wrapper .pagination .page_${Number(page) + 1}`).removeClass('d-none');
    $(`#${this.tableId}_wrapper .pagination .page_${Number(page) - 1}`).removeClass('d-none');
...
class Query {
    ...
    searchQuery(value) {
        return this.getQuery(`${this.mainCondition()} ${this.havingQuery(value)} ${this.groupByQuery()} ${this.orderByQuery()} limit 15`);
    }

    getAll(page = 0) {
        return run(this.getQuery(`${this.mainCondition()} ${this.groupByQuery()} ${this.orderByQuery()} ${this.limitQuery(page)}`)).result[0].query0;
    }
...
class Factory extends Query {
    constructor(table, tableLabel, fields, options = {}) {
        ...
        if (this.pageCondition() != '') {
            this.size = run(`${this.pageCondition()};`).result[0].query0[0].count ?? 0;
        } else {
            this.size = 0;
        }
        this.pages = [0];
    ...
    init() {
        ...
        <script>
            ...
            $('#${this.table}-table_wrapper input[type=search]').off().on('keyup', function(e) {
                ${this.table}.dataTable.search($(this).val()).draw();
                if(${this.table}.havingQuery('') != ''){
                    if($(this).val()){
                        let dataBaseLabs = run(${this.table}.searchQuery($(this).val())).result[0].query0;
                        ...
                        ${this.table}.resetPageForSearch(${run(`${this.pageCondition()};`).result[0].query0[0].count ?? 0});
                        customPagination.call(${this.table});
                    }
                }
            });
    ...
    pageCondition() {
        return `select count(*) as count from ${this.table} where lab_id='${localStorage.getItem('lab_hash')}'`;
    }
    ...
    getQuery(resetQuery) {
        return `SELECT ${this.fields.map(item => item.name).join(',')} FROM ${this.table} ${resetQuery};`;
    }
                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="${this.modalId}"></h5>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-main-add" id="${this.table}-save">Save changes</button>
                                </div>
                            </div>
                        </div>
                    </div>`;
    ...
    filterTable(user_q, start_date, end_date) {
        // get the data
        let data = run(this.getQuery(`${this.filterQuery(user_q, start_date, end_date)}`)).result[0].query0;
        // clear the table
        this.dataTable.clear();
        // add the rows
        this.createTableBody(data);
    ...
    fields.forEach(field => {
        switch (field.type) {
            ...
            case 'ignore':
                ...
                break;
            case 'custom':
                field.clearFormFun ? field.clearFormFun(field) : $(`#${formId} [name=${field.name}]`).val('');
                break;
            default:
                $(`#${formId} [name=${field.name}]`).val('')
                break;
...
function fillForm(formId, fields, item) {
    fields.forEach(field => {
        switch (field.type) {
            case 'select':
                $(`#${formId} [name=${field.name}]`).val(item[field.name]).trigger('change');
                break;
            case 'textarea':
            ...
            case 'image':
                ...
                window[`${field.name}_preview`].clearPreviewPanel();
                window[`${field.name}_preview`].imagePreview.style.backgroundImage = `url("${item[field.name]}")`;
                break;
            case 'checkbox':
            ...
            case 'ignore':
                break;
            case 'custom':
                field.fillFormFun ? field.fillFormFun(item) : $(`#${formId} [name=${field.name}]`).val(item[field.name]);
                break;
            default:
    ...
    fields.forEach(field => {
        switch (field.type) {
            ...
            case 'image':
                ...
                break;
            case 'checkbox':
                data[field.name] = $(`#${formId} [name=${field.name}]`).is(':checked') ? 1 : 0;
                break;
            case null:
                break;
            case 'ignore':
                break;
            case 'custom':
                field.getFormDataFun ? field.getFormDataFun(data) : data[field.name] = $(`#${formId} [name=${field.name}]`).val();
                break;
            default:
                data[field.name] = $(`#${formId} [name=${field.name}]`).val()
                break;
    ...
    fields.forEach(field => {
        switch (field.type) {
            ...
            case 'image':
                ...
                break;
            case 'checkbox':
                if (field.req && $(`#${formId} [name=${field.name}]`).is(':checked') == false) {
                    $(`#${formId} [name=${field.name}]`).addClass('is-invalid');
                    valid = false;
                } else {
                    $(`#${formId} [name=${field.name}]`).removeClass('is-invalid');
...
function normalInput(field) {
    return `<div class="form-group ${field?.size ? `col-md-${field?.size}` : 'col-md-12'}">
                <label for="${field.name}">${field.label}</label>
                <input type="${field.type}" class="form-control" id="${field.name}" name="${field.name}" ${field.req ? field.req : ''}>
            </div>`;
}

function checkboxInput(field) {
    return `<div class="form-group  ${field?.size ? `col-md-${field?.size}` : 'col-md-12'}">
                <label for="${field.name}" class="w-100 text-center">${field.label}</label>
                <label class="d-flex switch s-icons s-outline s-outline-success mx-auto mt-2">
                    <input type="checkbox" name="${field.name}" value="1" id="${field.name}" ${field.req ? field.req : ''}>
...
function selectInput(field) {
    ...
    return `<div class="form-group  ${field?.size ? `col-md-${field?.size}` : 'col-md-12'}">
                <label for="${field.name}">${field.label}</label>
                <select class="form-control" id="${field.name}" name="${field.name}" ${field.req ? field.req : ''}>
...
function fileInput(field) {
    ...
    return `<div class="form-group  ${field?.size ? `col-md-${field?.size}` : 'col-md-12'}">
                <div class="custom-file-container" data-upload-id="${field.name}">
                    <label>${field.label} <a href="javascript:void(0)" class="custom-file-container__image-clear" title="حذف الملف">x</a></label>
                    <label class="custom-file-container__custom-file" >
                        <input type="file" class="custom-file-container__custom-file__custom-file-input" accept="/*" ${field.req ? field.req : ''}>
                        <input type="hidden" name="MAX_FILE_SIZE" value="10485760" />
                        <span class="custom-file-container__custom-file__custom-file-control"></span>
                    </label>
                    <div class="custom-file-container__image-preview"></div>
                </div>
                <input dir="ltr" type="hidden" class="form-control" name="${field.name}" id="${field.name}"/>
            </div>
            `;
}
...
function fireSwal(fun, ...args) {
    let diffTimeInMin = -1 * ((new Date(localStorage?.getItem("lastSync") ?? "2023-01-01 00:00:00")) - new Date()) / 1000 / 60;
    if (diffTimeInMin > .1) {
        syncOnline();
...
function setTable(id = 'table', table, options = {}) {
    return $(`#${id}`).DataTable({
        ...
        "dom": "<'dt--top-section'<'row'<'col-sm-12 col-md-4 d-flex justify-content-md-start justify-content-center'B><'col-sm-12 col-md-4 d-flex justify-content-md-center justify-content-center'l><'col-sm-12 col-md-4 d-flex justify-content-md-end justify-content-center mt-md-0 mt-3'f>>>" +
            "<'table-responsive'tr>" +
        ...
        buttons: [
            ...
            { extend: 'excel', className: 'btn btn-sm btn-success d-none print-excel', text: '<i class="far fa-file-spreadsheet"></i> تصدير إكسيل' },
            { extend: 'print', className: 'btn btn-sm btn-primary d-none print-page', text: '<i class="far fa-print"></i> طباعة' },
            { className: 'btn btn-sm btn-success active-print-excel', text: '<i class="far fa-file-spreadsheet"></i> تصدير إكسيل' },
            { className: 'btn btn-sm btn-primary active-print-page', text: '<i class="far fa-print"></i> طباعة' },
...
function manageImageSave(imageId) {
    ...
    let value = null;
    if (ImageElement.val().includes(`${__domain__}app`) || ImageElement.val().includes(`${__domain__}lab`) || ImageElement.val().includes(`http://umc.native-code-iq.com/`)) {
...
function printElement(Id, pageZise = 'A4', ...args) {
    ...
    // add new stylesheet
    mywindow.document.write(args.map(arg => `<link rel="stylesheet" href="${arg}">`).join(''));
    ...
    `);
    // add bootstrap
    mywindow.document.write('<link href="bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />');
    mywindow.document.write('<link rel="stylesheet" href="plugins/font-awesome/css/all.css">');
    ...
    // add jquery
    mywindow.document.write('<script src="assets/js/jquery-3.5.1.min.js"></script>');
    // add bootstrap

    mywindow.document.write('<script src="bootstrap/js/bootstrap.min.js"></script>');
