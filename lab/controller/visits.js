...
function toggleHeaderAndFooter() {
    ...
    run(`update lab_invoice set footer_header_show='${invoiceShow == 0 ? 1 : 0}' where lab_hash='${localStorage.getItem('lab_hash')}';`);
...
function showPackagesList(hash) {
    ...
    $(this).popover({
        template: '<div class="popover popover-light" style="direction:ltr" role="tooltip"><div class="arrow"></div><h3 class="popover-header"></h3><div class="popover-body"></div></div>',
        ...
        content: `
            <div class="row">
                ...
                `<div class="col-md-12">
                    <p class="text-left">الجهاز: ${package.device_name ?? "No Device"}</p>
                </div>
                <div class="col-md-12">
                    <p class="text-left">الكت: ${package.kit_name ?? "No Kit"}</p>
                </div>
                `}
            </div>
        `,
...
function visitDetail(hash) {
    ...
    show_visit_button.attr('onclick', `fireSwalWithoutConfirm(showVisit,'${hash}')`);
    invoice_button.attr('onclick', `fireSwalWithoutConfirm(showInvoice,'${hash}')`);
    show_add_result.attr('onclick', `fireSwalWithoutConfirm(showAddResult,'${hash}')`);
}
        ...
        <div class="statbox widget box box-shadow bg-white py-3">
            ...
            <div class="custom-card visit-info">
                ...
                <div class="custom-card-body" dir="rtl">
                    <table class="information-1">
                        <tbody>
                            ...
                            <tr>
                                <td>العمر</td>
                                <td>${parseFloat(visit?.age).toFixed(2) ?? ''} سنة</td>
                            </tr>
                            <tr>
                                <td>التاريخ</td>
                            ...
                            <tr>
                                <td>اجمالي المبلغ</td>
                                <td>${visit?.total_price?.toLocaleString() ?? ''} IQD</td>
                            </tr>
                            <tr>
                                <td>صافي الدفع</td>
                                <td>${visit?.net_price?.toLocaleString() ?? ''} IQD</td>
                            </tr>
                            <tr>
                                <td>ملاحظات</td>
                                <td>${visit?.note ?? ''}</td>
                ...
                <div class="custom-card-footer d-print-none">
                    ...
                    ${!window.location.pathname.includes('history') ? `<div class="row mt-3">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="input-group my-2">
                                        <input type="text" class="form-control" id="patientPhone" placeholder="${patient.phone}" value="${patient.phone}">
                                        <div class="input-group-append">
                                            <button class="btn btn-add" type="button" onclick="updatePhone('${patient.hash}')">حفظ</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
function showAddResult(hash, animate = true) {
    ...
    getCurrentInvoice($(`#${localStorage.getItem('currentInvoice') == 'undefined' || !localStorage.getItem('currentInvoice') ? $('#invoice-tests-buttons').find('button').first().attr('id') : localStorage.getItem('currentInvoice')}`));
    ...
    $('.results select[multiple]').each(function () {
        ...
        $(this).on('select2:select', function (e) {
            if ($(this).val().length > 1) {
                $(this).val($(this).val().filter((v) => v.toUpperCase() != 'ABSENT'));
                $(this).trigger('change');
            }
        });
    });
    ...
    if (animate) {
        $('html, body').animate({
            scrollTop: ($("#main-space").offset().top * ($(window).width() < 2100 ? $(window).width() / 2100 : 1) - 75),
        }, 1000);
    }
...
function filterWithKit(reference, kit) {
    return reference.filter((ref) => {
        if ((kit == '' || kit == null || kit == undefined) || (ref.kit == '' || ref.kit == null || ref.kit == undefined)) {
            return true;
        }
        if (kit == ref.kit) {
            return true;
...
function filterWithUnit(reference, unit) {
    return reference.filter((ref) => {
        if (unit == ref.unit || ref.unit == '' || ref.unit == null || ref.unit == undefined) {
            return true;
        }
...
function filterWithGender(reference, gender) {
    return reference.filter((ref) => {
        ...
        if (gender.trim().replace("ي", "ى") == ref.gender.trim().replace("ي", "ى")) {
            return true;
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-3 text-center">
                ...
                <label class="d-inline switch s-icons s-outline s-outline-invoice-slider mr-5">
                    <input type="checkbox" id="check_normal_${test.hash}" name="check_normal_${test.hash}" ${resultList?.checked ?? true ? 'checked' : ''} onclick="toggleTest.call(this)">
            ...
            <div class="col-md-7 mb-3 text-center" dir="ltr">
                ...
                <h5 class="text-center">${reference?.[0]?.result == 'result' ? reference?.[0]?.right_options : manageRange(reference)}</h5>
            </div>
           
            <div class="col-md-5 mb-3">
                <div class="row">
                    <div class="col-md-4 text-center d-flex justify-content-center align-items-end">
                        <span class="">${reference?.[0]?.result?.trim() == 'result' ? "" : testType == 'normal' ? (test?.unit_name ?? '') : (units.find(item => reference?.[0]?.unit == item?.hash)?.name ?? '')}</span>
                    </div>
                    <div class="col-md-8">
            ...
            :
            `<input type="number" class="form-control result text-center" id="result_${test.hash}" name="${test.name}" placeholder="ادخل النتيجة" ${testType == 'calc' ? 'readonly' : ''} value="${testType == 'normal' ? resultList?.[test.name] : resultList}">`
        }
...
function addNormalResult(component, test, visit, result_test, options, resultForm, testType = 'normal') {
    ...
    if (options.result = 'number') {
        resultForm.push(generateFieldForTest(test, result_test, reference, testType));
    } else if (0) { }
    return resultForm;
}
...
function addStrcResult(component, test, result_test, resultForm) {
    ...
    let results = {};
    resultForm.push(`
    <div class="col-md-11 results test-${test.name.replace(/\s/g, '').replace(/[^a-zA-Z0-9]/g, '')} mb-15 ">
    ...
    ${component.map((comp) => {
        ...
        switch (comp.result) {
            case 'result':
                input = `
                <select class="form-control result text-center h6" ${editable} name="${comp.name}" id="result_${test.hash}" ${comp.multi === true ? 'multiple' : ''}>
            ...
            case 'nubmer':
                input =
                    `<input type="number" class="form-control result text-center" ${editable}
            ...
            default:
                input =
                    `<input type="text" class="form-control result text-center" ${editable}
                     value="${result}"
                     id="result_${test.hash}" name="${comp.name}"
                    placeholder="ادخل النتيجة">`
                break;
        ...
        return `
        ${typeDiff ? `<div class="col-md-12 text-center">${comp.type}</div>` : ''}
        <div class="${comp.type == "Notes" ? 'col-md-12' : 'col-md-4'} mb-3 text-left">
            <label for="result" class="w-100 text-center text-black font-weight-bold h5">${comp.name} ${comp.unit ? `(${comp.unit})` : ''}</label>
            ${input}
        </div>
        `;
...
function addResult(visit, visitTests) {
    ...
    let resultForm = [`<div class="col-11 my-3">
    ...
    visitTests.forEach((test) => {
        ...
        if (type == 'calc') {
            let result = 0;
            try {
                let equ = value.map(item => {
                    // check if item is number
                    if (!isNaN(item)) {
                        return item;
                    }
                    else if (!calcOperator.includes(item)) {
                        let finalResult = result_tests.find(test => test.name == item)?.result ?? 0;
            ...
            addNormalResult(component, test, visit, finalResult, options, resultForm, 'calc');
        }
        else if (type == 'type') {
            resultForm = addStrcResult(component, test, result_test, resultForm);
        }
        else {
            resultForm = addNormalResult(component, test, visit, result_test, options, resultForm);
        }
    });
...
function saveResult(hash) {
    ...
    $('.result').each(function () {
        ...
        let checked = $(`input[type=checkbox][name=check_normal_${_hash_}]`).is(':checked') ?? undefined;
...
function focusInput(type) {
    ...
    $('html, body').animate({
        scrollTop: list.eq(index).parents('.results').offset().top * ($(window).width() < 2100 ? $(window).width() / 2100 : 1) - 100
    }, 250, function () {
        list.eq(index).focus();
    });
...
function toggleCheckboxAndSelectedTest(hash) {
    let checkbox = document.querySelector(`input[type=checkbox][value="${hash}"]`);
    // click checkbox
    checkbox.click();
                        ...
                        <div class="header money">
                            <div class="row justify-content-between">
                                ...
                                <div class="right">
                                    <!-- عنوان جانب الايمن -->
                                    <div class="size1">
                                        <p class="title">${invoices?.name_in_invoice ?? localStorage?.lab_name ?? 'اسم التحليل'}</p>
                                        <p class="namet">${localStorage?.invoice_about_ar ?? 'للتحليلات المرضية المتقدمة'}</p>
                                        <p class="certificate">${localStorage?.invoice_about_en ?? 'Medical Lab for Pathological Analyses'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <script>
                            ...
                            <div class="tester">
                                <div class="row m-0">
                                    ...
                                    ${item.tests.split(',').map((test, index) => {
        if (item.name != test) {
            return `
                        ...
                        <div class="footer">
                            ...
                            <div class="f2">
                                ...
                                <p>${invoices?.address ? `<i class="fas fa-map-marker-alt"></i> ${invoices?.address}` : ''}
                                <span class="note">${invoices?.facebook == '' ? '' : `&nbsp;&nbsp;&nbsp;&nbsp;<i class="fas fa-envelope"></i>  ${invoices?.facebook}&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;`}</span>
                                
                                <span class="note">${invoices?.phone_1 == '' ? '' : `<i class="fas fa-phone"></i>  ${invoices?.phone_1}`}</span></p>
                            </div>
            ...
            <div class="header">
                <div class="row justify-content-around">
                    ...
                    <div class="logo justify-content-end col-4 p-2">
                        <!-- شعار التحليل -->
                        <h2 class="navbar-brand-name text-center">${invoices?.name_in_invoice ?? localStorage.lab_name ?? ''}</h2>
                    </div>
                </div>
            </div>
            `;
...
function createInvoice(visit, type, form) {
    ...
    return `<div class="book-result" dir="ltr" id="invoice-${type}" style="display: none;">
		<div class="page">
			<!-- صفحة يمكنك تكرارها -->
			${header}
			<div class="center2" ${invoices?.footer_header_show == 1 ? 'style="border-top:5px solid #2e3f4c;"' : 'style="border-top:none;"'}>
                <div class="center2-background"></div>
				<div class="nav">
					<!-- شريط تخصص التحليل -->
					<div class="name">
					...
					<div class="agesexgo">
						<p><span class="note">${visit.gender == 'ذكر' ? 'Male' : 'Female'}</span> / <span class="note">${parseFloat(visit.age) < 1 ? parseInt(visit.age * 356) + ' Day' : parseInt(visit.age) + ' Year'}</span></p>
					</div>
					<div class="vid">
						<p class="">Date</p>
					</div>
					<div class="vidgo">
						<p><span class="note">${visit.date}</span><!--&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<div class="prdgo">
						<p><span class="note">${visit.doctor == 'تحويل من مختبر اخر' ? '' : `${visit.doctor ?? ''}`}</span></p>
					</div>
				</div>
			...
			<div class="footer2" ${invoices?.footer_header_show == 1 ? 'style="border-top:5px solid #2e3f4c;"' : 'style="border-top:none;"'}>
				<div class="f1">
					<p>${invoices?.address ? `<i class="fas fa-map-marker-alt"></i> ${invoices?.address}` : ''}</p>
				</div>
				<div class="f2">
					<p><span class="note">${invoices?.facebook == '' ? '' : `&nbsp;&nbsp;&nbsp;&nbsp;<i class="fas fa-envelope"></i>  ${invoices?.facebook}&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;`}</span>
                    <span class="note">${invoices?.phone_1 == '' ? '' : `<i class="fas fa-phone"></i>  ${invoices?.phone_1}`}</span></p>
				</div>
			</div>
		</div>
...
function getNormalRange(finalResult = '', range = []) {
    let { normalRange, color, flag } = { normalRange: 'No Range', color: 'dark', flag: '' };
    let { name = '', low = '', high = '' } = range;
    if (low != '' && high != '') {
        normalRange = (name ? `${name} : ` : '') + low + ' - ' + high;
    } else if (low == '') {
...
function normalTestRange(finalResult = '', refrence) {
    ...
    switch (result) {
        case 'result':
            ...
            if (right_options) {
                returnResult.color = right_options.includes(finalResult) ? 'text-dark' : 'text-danger p-1 border border-dark';
...
function showResult(visit, visitTests) {
    ...
    visitTests.forEach((test, index) => {
        ...
        if (options.type == 'type') {
            ...
            buttons[test.name.replace(/\s/g, '').replace(/[^a-zA-Z0-9]/g, '')] =
                `<div class="col-auto">
                    <button class="action btn btn-action mx-2 w-100" id="test-${test.name.replace(/\s/g, '').replace(/[^a-zA-Z0-9]/g, '')}" onclick="getCurrentInvoice($(this))">${test.name}</button>
                </div>
                `;
            let invoiceBody = '';
            ...
            for (let reference of component) {
                ...
                let resultClass = '';
                let flag = '';
                let { low = '', high = '', infinit = '' } = reference?.reference[0]?.range[0];
                if (infinit != '' || (low && high)) {
                    ...
                    let resultCompare = result.toString().toUpperCase().replace(/\s+/g, "");
                    let defualtCompare = defualt.toString().toUpperCase().replace(/\s+/g, "");
                    if (resultCompare === defualtCompare || resultCompare === '' || resultCompare === 'ABSENT') {
                        resultClass = '';
                    } else {
            ...
            invoices[test.name.replace(/\s/g, '').replace(/[^a-zA-Z0-9]/g, '')] = invoiceBody;
        }
        else {
            ...
            if (category != test.category) {
                ...
                if (category) {
                    invoices.normalTests += `
                        <div class="test typetest pt-3 category_${category?.split(' ')?.join('_')}">
                            <p>${category}</p>
                        </div>
                        `
                };
            ...
            let result = 0;
            try {
                result = options?.type == 'calc' ? eval(value.map(item => {
                    if (!isNaN(item)) {
                        return item;
                    }
                    else if (!calcOperator.includes(item)) {
            ...
            let { color, normalRange, flag } = normalTestRange(result, reference?.[0]);
            invoices.normalTests += manageTestType('flag', {
                ...
                unit: `${reference?.[0]?.result == 'result' ? "" : options?.type != 'calc' ? (test?.unit_name ?? '') : (units.find(item => reference?.[0]?.unit == item?.hash)?.name ?? '')}`
            });
            // invoices.normalTests += `
    ...
    return {
        ...
        invoice: `${Object.entries(invoices).map(([key, value]) => {
...
function printAll() {
    new Promise((resolve, reject) => {
        ...
        let currentInvoice = localStorage.getItem('currentInvoice') == 'undefined' || !localStorage.getItem('currentInvoice') ? $('#invoice-tests-buttons').find('button').first().attr('id') : localStorage.getItem('currentInvoice');
...
function printOneInvoice(id = null) {
    new Promise((resolve, reject) => {
        ...
        if (id) {
            ...
            printElement(`#invoice-${$('.book-result').first().attr('id').split('-')[1]}`, 'A4', 'css/invoice.css');
        }
...
function manageInvoiceHeight(invoiceId = null) {
    const elementsWithClasses = document.querySelectorAll('.typetest[class*="category_"]');
    const categoryClasses = Array.from(elementsWithClasses, element => element.classList.value.split(' ').find(cls => cls.startsWith('category_')));
    ...
    if (invoiceId) {
        ...
        $('.book-result:visible .page .center2 .tester .test:visible').each(function () {
            let eleHeight = $(this).outerHeight();
            allTestsElements.push({
                html: $(this).clone(),
                eleHeight
    ...
    let invoices = addTestToInvoice(center2Scroll, allTestsElements, cloneInvoice, center2Scroll);
    if (invoiceId) {
        $(`#${invoiceId}`).empty();
...
function addTestToInvoice(allInvoiceTestsHeight, allTestsElements, cloneInvoice, center2Scroll, lastTestType = null) {
    ...
    for (let i = 1; i <= invoiceCount; i++) {
        ...
        for (let [index, test] of allTestsElements.entries()) {
            if (index == 0 && !test?.html?.hasClass('testhead') && allTestsElements?.[1]?.html) {
    ...
    if (allTestsElements.length > 0) {
        invoices = [...invoices, ...addTestToInvoice(center2Scroll, allTestsElements, cloneInvoice, center2Scroll, lastTestType)]
    }
    return invoices;
}
...
function sendByWhatsapp(hash, phone) {
    ...
    if (phone?.length > 10) {
        ...
        let url = `https://api.whatsapp.com/send?phone=${phone}&text=http://umc.native-code-iq.com/lab/downloadInvoice.html?pk=${hash}-${localStorage.getItem('lab_hash')}`;
...
function manageTestType(type, test = {}) {
    let { name, color, result, hash, category, checked, normal, unit, flag, font, history } = test;
    switch (type) {
        case 'flag':
            return `
            <div data-flag="flag" class="test row m-0 category_${category?.split(' ')?.join('_')} border-test" id="test_normal_${hash}" data-cat="${category?.split(' ')?.join('_')}" style="display:${checked}">
                <div class="testname col-3">
                ...
                ${history != "" && history && history != "{}" ? `<div class="testprice col-12 h5 text-right text-info">
