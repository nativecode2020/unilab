...
async function getKitsUnitsByTestName(testName, testHash) {
    ...
    return await fetch(`${__domain__}app/index.php/Packages/getKitsAndUnitsByTestName`, {
        method: 'POST',
        headers: {
...
function changeCost(hash) {
    const element = document.querySelector(`#row-packages input[type=checkbox][value="${hash}"]`);
    const searchPackageElement = document.querySelector('#search_package #testSearch');
    ...
    if (document.querySelectorAll(`input[type=checkbox][value="${hash}"]`).length <= 1) {
        console.log('here');
        document.querySelector('#addPackage #cost').value = cost + currentCost;
        if (!document.querySelector(`#selected-tests input[type=checkbox][value="${hash}"]`)) {
        ...
        document.querySelector(`#selected-tests input[type=checkbox][value="${hash}"]`).parentElement.parentElement.remove();
        element.checked = false;
    }
}

async function changeKitsAndUnits(name, hash) {
    let __data = await getKitsUnitsByTestName(name, hash).then(res => res.json());
    let testKitsByName = __data.data.kits;
    // delete duplicate kits with same name
    testKitsByName = testKitsByName.filter((thing, index, self) =>
    ...
    testUnitsByName = testUnitsByName.filter((thing, index, self) =>
        index === self.findIndex((t) => (
            t.name === thing.name
        ))
    );
    ...
    if (testKitsByName.length > 0) {
        let kitsIdList = testKitsByName.filter(item => item.kit_id).map(item => {
            if (item.kit_id) {
                return item.kit_id
            }
            return false
        ...
        if (kitsIdList.length != 0) {
            ...
            select 
                ...
                from 
                    lab_device 
                inner join  devices on lab_device.devices_id = devices.id and lab_device.lab_id = '${localStorage.getItem('lab_hash')}}'  
    ...
    $('#kit_id').empty();
    $('#unit').empty();
    $('#unit').append(testUnitsByName.map((item, index) => `<option value="${item.unit_id}">${item.name}</option>`));
    $('#lab_device_id').append(testDevicesByKit.map((item, index) => `<option value="${item.id}">${item.name}</option>`));
    $('#kit_id').append(testKitsByName.map((item, index) => `<option value="${item.kit_id}">${item.name}</option>`));
...
function addKitsByDevice(ele) {
    ...
    let deviceKits = devices_kits.filter(item => item.device_id == device_id).map(item => item.kit_id);
    ...
    kitElement.append(kitsResult.map(item => `<option value="${item.id}">${item.name}</option>`).join(''));
...
function addUnitsByKit(ele) {
    if (1) return false;
    let kit = $(ele).val();
    let kitUnits = kit_units.filter(item => item.kit == kit).map(item => item.unit);
    ...
    unitElement.append(unitsResult.map(item => `<option value="${item.hash}">${item.name}</option>`).join(''));
...
function getDeviceByKit(id) {
    let devices_id = devices_kits.filter(item => item.kit_id == id).map(item => item.device_id);
    // get devicesbykit from local storage
    let testDevicesByKit = JSON.parse(localStorage?.getItem('testDevicesByKit'));
    let device = testDevicesByKit.filter(item => devices_id.find(x => x == item.id));
    return device[0]?.id;
}
...
async function updateTest(hash) {
    ...
    $('#addTest #lab_device_id').val(test.lab_device_id ?? '0').trigger('change');
    $('#addTest #kit_id').val(test.kit_id).trigger('change');
    $('#addTest #unit').val(test.unit ?? '0').trigger('change');
    $('.buttons .btn-action').removeClass('active');
    $('.buttons .btn-action[data-id="addTest"]').addClass('active');
    $('.page').hide();
    $('#addTest').show();
    // change save button
    $('#addTest #save-button').attr('onclick', `fireSwal(saveUpdateTest, '${hash}')`);
    // change title
...
function saveUpdateTest(hash) {
    ...
    let oldTest = run(`select lab_device_id,kit_id,unit,test_id from lab_pakage_tests where package_id='${hash}';`).result[0].query0[0];
    // update lab_package
...
function saveNewTest() {
    ...
    let newPackage = run_both(`insert into lab_pakage_tests(test_id,lab_device_id,kit_id,unit, package_id,lab_id) values('${hash}','${lab_device_id}','${kit_id}','${unit}', '${packageHash}','${localStorage.lab_hash}');` + allPAckages.getQuery(allPAckages.itemQuery(packageHash))).result[1].query1[0];
    allPAckages.addRow(newPackage);
    // empty form
    emptyTestForm()
    niceSwal('success', 'top-end', 'تم الحفظ بنجاح');
    let newObject = lab_test.getItem(packageHash);
...
function updatePackage(hash) {
    $('#selected-tests').empty();
    let data = run_both(`select name,price,cost,note from lab_package where hash='${hash}';
    ...
    tests.forEach(item => {
        $(`#row-packages input[type=checkbox][data-test="${item.test_id}"][data-device="${item.lab_device_id}"]`).click();
    });
    ...
    $('#addPackage').show();
    // change save button
    $('#addPackage #save-package').attr('onclick', `fireSwal(saveUpdatePackage, '${hash}')`);
    // change title
