...
let getData = run(`SELECT name,birth,hash FROM lab_patient WHERE lab_id='${localStorage.getItem('lab_hash')}' and isdeleted='0';
                   SELECT name,hash FROM lab_doctor WHERE lab_id='${localStorage.getItem('lab_hash')}' and isdeleted='0';
                   ...
                   SELECT name,jop, jop_en from lab_invoice_worker where lab_hash='${localStorage.getItem('lab_hash')}' and is_available=1 and isdeleted=0 limit 5;
                   select * from lab_invoice where lab_hash='${localStorage.getItem('lab_hash')}';`);
...
function getBirthByAge(age_year = 0, age_month = 0, age_day = 0) {
    let ageByDay = Number(age_year) * 365 + Number(age_month) * 30 + Number(age_day);
    let birth = new Date();
...
class Visit extends Factory {
    ...
    addRow(row) {
        if (this.dataTable.row(`#${row.hash}`)[0].length == 0) {
            let node = this.dataTable.row.add({
    ...
    resetForm() {
        ...
        $(`#${this.table}-save`).attr('onclick', `fireSwal.call(${this.table},${this.table}.saveNewItem)`);
    ...
    updateItem(hash) {
        ...
        // fill form with item
        let items = run(this.getItem(hash) + `
    ...
    validate() {
        ...
        if (patientHash == 'false' || patientHash == '0' || patientHash == '') {
            ...
            return false;
        } else if (parseInt($('#age_year').val()) * 356 + parseInt($('#age_day').val()) + parseInt($('#age_month').val()) * 30 <= 0) {
    ...
    getNewData() {
        ...
        let age = (Number(age_year) + Number(age_month / 12) + Number(age_day / 365)).toFixed(2);
        let name = ($('input[name=new_patient]').is(':checked') ? true : false) ? $('#visits_patient_id').val() : $(`#visits_patient_id option:selected`).html();
        let data = {
    ...
    saveNewItem() {
        ...
        from 
            lab_pakage_tests where package_id in (${insertedPackages});`).result[0].query0;
        // delete duplicate tests
        let error = false;
        insertedTests = insertedTests.filter((test, index, array) => {
            return array.findIndex((foundedTest) => {
                if (foundedTest.id === test.id && foundedTest.catigory_id != test.catigory_id) {
                    niceSwal('error', 'top-end', `التحليل ${test.name} مكرر`);
                    error = true;
                }
                return (foundedTest.id === test.id && foundedTest.catigory_id === test.catigory_id);
            }) === index;
        });
        if (error) {
            return false;
        ...
        $('.testSelect:checked').each(function () {
            mainQuery += `insert into lab_visits_package(visit_id, package_id, price,lab_id) values('${newObjectHash}', '${$(this).val()}', '${$(this).data('price')}','${localStorage.lab_hash}'); `;
        });
        ...
        $('#show_selected_tests div').remove();
        add_calc_tests(insertedTests.map(test => test.id), newObjectHash);
    ...
    saveUpdateItem(hash) {
        ...
        insertedTests = insertedTests.filter((test, index, array) => {
            return array.findIndex((foundedTest) => {
                if ((foundedTest.id === test.id && foundedTest.package_id === test.package_id) && foundedTest.catigory_id != test.catigory_id) {
                    niceSwal('error', 'top-end', `التحليل ${test.name} مكرر`);
                    error = true;
                }
                return ((foundedTest.id === test.id && foundedTest.package_id === test.package_id) && foundedTest.catigory_id === test.catigory_id);
            }) === index;
        });
        ...
        let mainQuery = `update lab_patient set ${Object.entries(patientData).map(([key, value]) => `${key}='${value}'`).join(',')} where hash = '${data.visits_patient_id}'; `
        mainQuery += `update ${this.table} set ${Object.entries(visitData).map(([key, value]) => `${key}='${value}'`).join(',')} where hash = '${hash}'; `;
        ...
        const deletedPackages = oldPackages.filter(_package => !newPackages.includes(_package));
        const deletedTests = oldTests.filter(_test => !newTests.some(newTest => newTest.package_id === _test.package_id && newTest.id === _test.tests_id));
        const addedPackages = newPackages.filter(_package => !oldPackages.includes(_package));
        const addedTests = newTests.filter(_test => !oldTests.find(_oldTest => _oldTest.package_id === _test.package_id && _oldTest.tests_id === _test.id));
        console.log("newtests=>", newTests, "oldtests=>", oldTests, "addedTests=>", addedTests);
        mainQuery += deletedPackages.map(_package => `delete from lab_visits_package where visit_id = '${hash}' and package_id = '${_package}'; `).join('');
        mainQuery += deletedTests.map(_test => `delete from lab_visits_tests where visit_id = '${hash}' and package_id = '${_test.package_id}' and tests_id = '${_test.tests_id}'; `).join('');
        mainQuery += addedPackages.map(_package => `insert into lab_visits_package(visit_id, package_id, price,lab_id) values('${hash}', '${_package}', '${$(`.testSelect[value=${_package}]`).data('price')}','${localStorage.lab_hash}'); `).join('');
        mainQuery += addedTests.map(_test => `insert into lab_visits_tests(visit_id, package_id, tests_id, lab_id) values('${hash}', '${_test.package_id}', '${_test.id}','${localStorage.lab_hash}'); `).join('');
    ...
    createModal() {
        let modal = `<div class="modal fade" id = "${this.modalId}" tabindex="-1" role="dialog" aria-labelledby="${this.modalId}" aria-hidden="true" >
                        ...
                        <div class="col-3">
                            ...
                            <div class="form-group" id="doctor_hash-form">
                                ...
                                <option value="false">اختر الطبيب</option>
                                    ${doctors.map(doctor => `<option value="${doctor.hash}">${doctor.name}</option>`).join('')}
                                </select>
                                <script>
                                    $('#doctor_hash').select2({
                                        width: '100%'
                        ...
                        <div class="col-md-12">
                            <div class="searchable-container packages-search">
                                <div class="searchable-items my-3 border-0" id="offers">
                                    ${packages.filter(item => item.type == '9').map(item => {
            return `
                                    <div class="n-chk item text-left mb-3">
                                    <label class="new-control items offer new-checkbox new-checkbox-rounded checkbox-outline-success font-weight-bolder mb-0" onmouseover="showPackagesList.call(this, ${item.hash})" onmouseleave="$(this).popover('hide')">
                                        <!--
                                    (<span class="text-danger w-100">${item.kit_name}</span>)

                                        -->
                                        <input type="checkbox" onclick="changeTotalPrice('${item.hash}')" class="new-control-input testSelect" data-name="${item.name}" data-price="${item.price}" value="${item.hash}" id="package_${item.hash}" >
                                        <span class="new-control-indicator m-3 "></span><span class="ml-4">${item.name}</span><p class="">IQD ${parseInt(item.price)?.toLocaleString()} </p>
                                    </label>
                                </div>
                                    `}).join('')}
                                </div>
                ...
                <div class="col-6 h-75" style="overflow-y: scroll;">
                    <div class="row justify-content-between">
                        ...
                        <div class="col-md-12">
                            <div class="searchable-container packages-search">
                                <div class="searchable-items my-3 border-0" id="offers">
                                    ${packages.filter(item => item.type != '9').filter((value, index, self) => index === self.findIndex(
                (t) => t.name === value.name
            )).map(item => `
    ...
    <div class="modal-footer">
        <button type="button" class="btn btn-main-add" onclick="fireSwal.call(${this.table},${this.table}.saveNewItem)" id="${this.table}-save">حفظ</button>
    </div>
    `
...
class Patient extends Factory {
    ...
    getItem(hash) {
        return run(`select * from ${this.table} where hash = '${hash}'; `).result[0].query0[0];
    }
    ...
    saveNewItem() {
        ...
        let newPatient = run(`select name, birth, hash from lab_patient where hash = '${newObjectHash}'; `).result[0].query0[0];
        patients.push(newPatient);
        $('#visits_patient_id').append(`< option value = "${newObjectHash}" > ${data.name}</ > `);
...
class Doctor extends Factory {
    ...
    saveNewItem() {
        ...
        let newDoctor = run(`select name, hash FROM lab_doctor where hash = '${newObjectHash}'; `).result[0].query0[0];
        doctors.push(newDoctor);
        $('#doctor_hash').append(`< option value = "${newObjectHash}" > ${data.name}</ > `);
